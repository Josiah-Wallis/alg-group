...
> * Abstract Factory Method: Our project uses a factory pattern to select and decide what operation is being used. Our factory consists of an abstract operation factory that will create concrete operations such as add, mult, sub, div, pow, and a custom operation inputted by the user. We can have the five basic operations or a custom operation given by the user. Our operation class will be able to create abstract versions without knowing the concrete implementation of parse(), which will parse through a given string and help form a custom operation given by a user. The parse function takes in two values as a Base pointer as well as the operation as a string. The implementation of parse will account for any operation the user inputs.
