...
## Class Diagram
 ...
 > * Strategy Method: Within the project, we want to be able to have a function verify() and function setGroup() both of which are in the Verifier Class (Compositor). The verify() checks if the input fits the requirements for a group, based on definition, and returns true or false based on if said requirements are met. For example, for Inverse, verify() would check if an inverse exists, for Associative, verify() would check if the associative property applies, and under Commutative, verify() would check if the community property is valid. Then the setGroup() function is able to dynamically allocate the group which makes it accessible in the subclasses: Associative, Communitive, Inverse, Closure, Identity, and others depending on the user needs. This should pass in a Group * group which should be the group the subclasses check. All subclasses should also include the bool verify().
